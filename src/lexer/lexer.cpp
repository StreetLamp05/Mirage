...
// keyword table
// if found: keyword token; else: IDENTIFIER.
...
private:
    ...
    // look ahead w/o consuming, returns '\0' if at end
    char peek(size_t offset = 0) const {
        size_t idx = pos_ + offset;
        return idx < source_.size() ? source_[idx] : '\0';
    }

    // consume the curr char and advance pos/line/col
    char advance() {
        char c = source_[pos_++];
        if (c == '\n') {
            line_++;
    ...
    // consume current char if it matches `expected`
    ...
    void skip_whitespace_and_comments() {
        while (!at_end()) {
            ...
            break; // not whitespace or comment -> done
        }
    }

    // identifiers and keywords
    // Read [a-zA-Z_][a-zA-Z0-9_]*, then check the keyword table.
    ...
    Token lex_identifier() {
        ...
        if (it != keywords.end()) {
            return make_token_at(it->second, loc, word);
        }
        return make_token_at(TokenType::IDENTIFIER, loc, word);
    }

    // numbers
    // handles: decimal ints, floats (with decimal point and/or exponent), hex (0x...), binary (0b...)
    ...
    Token lex_number() {
        ...
        if (current() == '0' && (peek(1) == 'x' || peek(1) == 'X')) {
            num += advance(); // '0'
            num += advance(); // 'x'
            if (at_end() || !std::isxdigit(current()))
        ...
        return make_token_at(type, loc, num);
    }

    // strings
    // regular strings: "..." w/ escape sequences
    ...
    Token lex_string() {
        ...
        if (peek(1) == '"' && peek(2) == '"') {
            return lex_multiline_string(loc);
        }
        ...
        while (!at_end() && current() != '"') {
            ...
            if (current() == '\\') {
                ...
                switch (current()) {
                    ...
                    default:
                        error(std::string("unknown escape sequence: \\") + current());
                }
                advance();
            } else {
                value += advance();
    ...
    Token next_token() {
        ...
        // tries to match the longest operator first
        ...
        switch (c) {
            ...
            // slash: / /= (// and /* handled in skip_whitespace_and_comments)
            case '/':
